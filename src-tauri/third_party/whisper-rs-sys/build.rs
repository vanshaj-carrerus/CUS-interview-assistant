...
fn generate_bindings(bindings_path: &std::path::Path) {
    if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() {
        ...
        sanitize_windows_bindings(bindings_path);
        patch_windows_enum_aliases(bindings_path);
        return;
    }
    ...
    #[cfg(feature = "metal")]
    {
        builder = builder.header("whisper.cpp/ggml/include/ggml-metal.h");
    }
    #[cfg(feature = "vulkan")]
    {
        builder = builder
    ...
    // Skip glibc stdio from bundled headers; emit MSVC-friendly whisper/ggml FFI only.
        ...
        .generate()
    {
        ...
        Err(e) => {
            ...
            std::fs::copy("src/bindings.rs", bindings_path).expect("Unable to copy bindings.rs");
            sanitize_windows_bindings(bindings_path);
            patch_windows_enum_aliases(bindings_path);
        }
    }
...
/// Bindgen on Windows often emits enum aliases as `c_uint`; whisper-rs expects `c_int`/`i32`.
fn patch_windows_enum_aliases(path: &std::path::Path) {
    if !target_is_windows() {
        return;
    }
    ...
    let patched = content
        ...
        .map(|line| {
            ...
            let is_whisper_or_ggml_enum_alias = line.starts_with("pub type ggml_")
                || line.starts_with("pub type whisper_");
            if is_whisper_or_ggml_enum_alias && line.contains("::std::os::raw::c_uint") {
    ...
    std::fs::write(path, patched).expect("Failed to patch Windows enum aliases in bindings.rs");
}

/// Bundled bindgen output includes glibc stdio layout tests that fail on Windows MSVC.
fn sanitize_windows_bindings(path: &std::path::Path) {
    if !target_is_windows() {
        return;
    }
    ...
    let patched = content
        ...
        .map(|line| {
            let line = line.trim_end_matches('\r');
            let is_layout_assert = (line.contains("size_of::<")
                || line.contains("align_of::<")
                || line.contains("offset_of!"))
                && line.contains(" - ")
                && line.contains("usize]");
            if is_layout_assert && glibc_stdio_markers.iter().any(|m| line.contains(m)) {
                "    [0usize]; // patched for MSVC".to_string()
            } else {
                line.to_string()
            }
        })
